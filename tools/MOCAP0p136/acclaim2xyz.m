...
xyzStruct(ind).rot = torientInv*tdof*torient*xyzStruct(parent).rot;  %~ key transformation here: AMC motion is applied in child coordinate, but the overall transformation is in parent coordinate
xyzStruct(ind).xyz = xyzStruct(parent).xyz + (skel.tree(ind).offset*xyzStruct(ind).rot); %~ get child position in parent coordinate system
