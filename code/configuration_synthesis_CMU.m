...
Qd = 3; % number of state for discriminator
Q = [3 3 6]; % use different state for different class, only for generator
M = 1; % number of mixture
num_params = 10;
num_sample = 10;
llh_train = 0; % whether compute the likelihood of training data using learned model
viterbi = 1; % set 1 if using decoded state to generate data, by default, the first training sequence state will be used
...
trial = 1;
savestep = 3; % how often to save checkpoint
N_ = 1000; % number of samples to synthesize

% hyperparameters for learning
reload = 0; %120 checkpoint: set this to non-zero to load an existing model
max_iters = 120;  %120 5 number of epoches
batch_size = 8; %64
